...
function MessageCard({ type, message, subMessage,setError }) {
  ...
  const colorMap = {
    error: {
      textColor: "#e53e3e", // red-600
      bgColor: "#fed7d7",   // red-100
      iconColor: "#e53e3e", // red-600
    },
    warning: {
      textColor: "#d69e2e", // yellow-500
      bgColor: "#faf089",   // yellow-100
      iconColor: "#d69e2e", // yellow-500
    },
    info: {
      textColor: "#3182ce", // blue-500
  ...
  const { textColor, bgColor, iconColor } = colorMap[type] || colorMap["info"];
  ...
  return (
    ...
    >
      ...
      {/* Icon Container */}
      <div
        className="w-[35px] h-[35px] flex justify-center items-center rounded-full ml-2.5"
      ...
      <div className="flex flex-col justify-center items-start flex-grow">
        <p
          className="m-0 cursor-default text-[17px] font-semibold"
          style={{ color: textColor }}
        >
          {message}
        </p>
        <p className="text-[14px] text-[#555]">{subMessage}</p>
      </div>

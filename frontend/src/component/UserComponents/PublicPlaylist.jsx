...
function PublicPlaylist() {
    ...
    const handlePlaylistVisibility = async (visibility) => {
        const toastId = toast.loading(`Making Playlist ${visibility ? "Private" : "Public"}...`);
        try {
            const isPublic = !visibility;
            const res = await API.put(`/playlists/${id}/updateVisibility`, { visibility: isPublic });
            if (res.status === 200) {
                toast.success(res.data.message, { id: toastId });
                getPlaylistDetails()
            }
        } catch (error) {
            console.error("Error in changing playlist visibility!", error);
            toast.error("Error in changing playlist visibility!", { id: toastId });
        }
    }
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <img
                            ...
                            src={`${import.meta.env.VITE_BASEURL}/assets/users/${playlist.user_id.profilePic}`} // Replace with actual image URL
